...
> ⚠️ **Segurança de credenciais:** nunca deixe e‑mail e senha fixos ("hardcoded")
> no código-fonte. Prefira variáveis de ambiente ou o helper `credentials()`.

### Carregando credenciais de forma segura

O helper `pyquotex.config.credentials()` resolve as credenciais nesta ordem:

1. Variáveis de ambiente `PYQUOTEX_EMAIL` e `PYQUOTEX_PASSWORD`
   (recomendado para servidores/CI — nada é gravado em disco).
2. Arquivo `settings/config.ini` (já incluído no `.gitignore`).
3. Prompt interativo, que grava o arquivo com permissões `0600`
   (a senha é lida sem eco na tela via `getpass`).
...
cliente = Quotex(email=email, password=password, lang="pt")
```

```bash
# Exemplo definindo variáveis de ambiente
export PYQUOTEX_EMAIL="seu_email@gmail.com"
export PYQUOTEX_PASSWORD="sua_senha"
```
...
## Gestão de Estado (Enums)

O PyQuotex utiliza `IntEnum` para um rastreamento de estado robusto. Isso substitui as antigas flags inteiras/booleanas
por estados claros e descritivos.

### Status do WebSocket

Disponível em `client.api.state.status`:
...
Disponível em `client.api.state.auth_status`:
